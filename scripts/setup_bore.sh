...
BORE_URL_GRAFANA=$(cat ${ROOT}/bore_grafana.out | grep at | cut -d ' ' -f7)
...
echo "BORE_URL_GRAFANA=${BORE_URL_GRAFANA}"
...
if [ -z "${GITHUB_OUTPUT}" ]; then
  ...
  echo "BORE_URL_GRAFANA=${BORE_URL_GRAFANA}" >> ${GITHUB_OUTPUT}
